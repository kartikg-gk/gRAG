...
# ==========================================================================
# Issue paging is bounded
# ==========================================================================


def test_a_limit_bounds_requests_not_only_results():
    """A tracker holding no issues must not be walked indefinitely.

    The pull-request filter sits between pagination and the limit, so a limit
    counts survivors while pagination keeps pulling pages. On a tracker whose
    pages are all pull requests there is no survivor to stop on, and the walk
    only ends when the repository does.
    """
    recorder: list[httpx.Request] = []
    pages = [[issue_payload(n, is_pull_request=True)] for n in range(1, 41)]
    session = session_with_pages(pages, recorder)

    issues = list(fetch_issues(session, "o/r", limit=1))

    assert issues == []
    assert len(recorder) <= MAX_ISSUE_PAGES


def test_issue_paging_stops_as_soon_as_the_limit_is_filled():
    """The bound must not cost a request when issues are found straight away."""
    recorder: list[httpx.Request] = []
    pages = [[issue_payload(1), issue_payload(2)]] + [
        [issue_payload(n)] for n in range(3, 20)
    ]
    session = session_with_pages(pages, recorder)
...
def test_the_page_bound_applies_to_issues_only():
    """Pull requests and commits carry no client-side filter, so nothing to bound."""
    recorder: list[httpx.Request] = []
    pages = [[pr_payload(n)] for n in range(1, 41)]
    session = session_with_pages(pages, recorder)

    pulls = list(fetch_pull_requests(session, "o/r"))
