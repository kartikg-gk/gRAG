...
Arrival order reaches the output: entity labels are first-seen-wins, and runs
are diffed against committed output. So the ordering has to be a property of
the data, not of the order GitHub happened to return things in.
...
def test_nothing_in_the_ingest_path_sorts_fetched_items_itself():
    ...
    for path in [
        ...
        *(root / "src" / "ingestion").glob("*.py"),
        ...
        if path.name in {"order.py", "source.py"}:
            # Source indexing ranks file paths against a question; it does not
            # reorder GitHub event ingestion, which remains centralized here.
            continue
