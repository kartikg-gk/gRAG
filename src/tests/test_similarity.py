...
def test_a_store_from_a_different_embedder_is_refused():
    ...
    **A dimension check cannot detect a different embedder of equal
    dimension.** The payload below is 384 floats, exactly what this scorer
    declares, so every length check in the module passes it. Only the identity
    separates it from a store this embedder wrote — which makes this test the
    one thing standing between a changed embedder and scores that are the
    right shape, plausible to read, and meaningless.

    The two vectors would score against each other without complaint. Nothing
    ...
    # The dimension is genuinely equal, so a length check is no defence here.
    assert stale["dimension"] == similarity.dimension
    assert len(stale["vectors"]["payment_service"]) == similarity.dimension

    with pytest.raises(ValueError, match="stale"):
        similarity.load_vectors(stale)


def test_the_same_embedder_is_accepted(tmp_path):
    """The other half of the identity check: a matching store loads.

    Asserted separately from the round-trip test because a check that refused
    everything would pass every rejection test in this file.
    """
    source = Similarity()
    source.vector("payment_service")
    ...
    assert target.export_vectors()["identity"] == source.export_vectors()["identity"]
    assert "payment_service" in target.export_vectors()["vectors"]


def test_the_rejection_says_which_check_failed():
    """A caller must be able to tell the two failures apart from the message.

    "Stale store" covers two unrelated causes — the embedder changed, or the
    dimension changed — and they need different fixes: re-embed the corpus, or
    correct a declared dimension. A message naming neither sends the reader to
    the source to find out which happened.
    """
    similarity = Similarity()
    matching_identity = SentenceTransformerEmbedder().identity

    with pytest.raises(ValueError) as changed_embedder:
        similarity.load_vectors(
            {
                "identity": "all-MiniLM-L12-v2-384",
                "vectors": {"payment_service": [0.0] * 384},
    ...
    with pytest.raises(ValueError) as changed_dimension:
        similarity.load_vectors(
            {
    ...
    embedder_message = str(changed_embedder.value)
    dimension_message = str(changed_dimension.value)

    # The identity failure names both identities, so the reader can see what
    # wrote the store and what is reading it.
    assert "all-MiniLM-L12-v2-384" in embedder_message
    assert matching_identity in embedder_message
    assert "declared dimension" not in embedder_message

    # The dimension failure names the length it got and the one it wanted.
    assert "declared dimension" in dimension_message
    assert "384" in dimension_message
    assert "stale" not in dimension_message

    assert embedder_message != dimension_message
...
# merges that are wrong on this corpus, and a test that hid them would be worse
# than no test. Adjusting a threshold or special-casing ticket-shaped strings
# would both bury the finding.
# --------------------------------------------------------------------------
