...
def clean_checkout(tmp_path_factory) -> Path:
    ...
    # The UI is a build input, produced by the separate frontend build gate.
    ui = PROJECT_ROOT / "src" / "tracing" / "ui"
    assert (ui / "viewer.html").is_file(), "Build and bundle the frontend before package tests"
    shutil.copytree(ui, export / "src" / "tracing" / "ui", dirs_exist_ok=True)
...
def test_the_console_command_renders_a_trace(clean_install):
    ...
    process = subprocess.Popen(
        [str(command), str(PROJECT_ROOT / "example_trace.json"), "--port", "0", "--no-browser"],
        stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True,
    )
    executor = ThreadPoolExecutor(max_workers=1)
    try:
        line = executor.submit(process.stdout.readline).result(timeout=15)
        assert "http://127.0.0.1:" in line
        address = line.split(" at ", 1)[1].strip()
        with urlopen(address + "api/local/traces/0", timeout=5) as response:
...
# --------------------------------------------------------------------------
#
# There was a test here pinning the runtime set to an exact list, so that a
# new runtime dependency had to be argued for before it could land. It is
# gone, and nothing replaces it.
#
# It was removed rather than widened because the rule it enforced and the
# control plane are incompatible: a control plane several processes share
# needs an ORM and a database driver, and no list-of-two survives that. A
# widened list would have kept the shape of the rule while conceding the thing
# it protected, which is a worse outcome than removing it — the check below
# still keeps test-only and example-only packages out of the runtime set, and
# the runtime set itself is now argued for in pyproject.toml rather than
# asserted here.
