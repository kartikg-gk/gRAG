...
def test_the_artifact_is_json_the_viewer_understands(tmp_path):
    ...
    assert payload["graph"]["nodes"] and payload["metrics"]
...
def test_retrieval_is_ordered_by_score_then_id():
    # Scoring divides by the node's own token count, so "alpha charlie" — one
    # of two tokens matched — scores 0.5 while "alpha bravo" scores 1.0. The
    # two 0.5s then have to be separated by id, which is what this pins.
    nodes = {
        "b:1": {"id": "b:1", "title": "alpha charlie"},
        "a:1": {"id": "a:1", "title": "alpha charlie"},
        "c:1": {"id": "c:1", "title": "alpha bravo"},
    }

    items, _ = retrieve(nodes, [], "alpha bravo")

    assert [item["score"] for item in items] == [1.0, 0.5, 0.5]
