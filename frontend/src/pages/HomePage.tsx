  ...
  return (
    <section className="dotted-canvas flex h-full min-w-0 flex-col bg-paper" aria-label="Graph workspace">
      ...
      <div className="relative min-h-0 flex-1" aria-label="Trace graph canvas">
        ...
        {trace.id === "trace_empty" && !retrieving ? <div className="pointer-events-none absolute inset-0 flex items-center justify-center"><p className="rounded-sm border border-line bg-panel/90 px-4 py-2 text-sm text-ink-muted">Run a trace to explore the graph.</p></div> : null}
        {answer !== null ? (
